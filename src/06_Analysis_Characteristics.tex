...
In challenges such as the ImageNet classification challenge (\cite{DBLP:journals/corr/RussakovskyDSKSMHKKBBF14}) the ultimate goal is to achieve the highest accuracy possible, neglecting other performance metrics like inference time. \cite{DBLP:journals/corr/CanzianiPC16}\\
...
\section{Datasets and models used}\label{sec:data_models}
In all experiments we are going to use some of the most common neural network architectures, namely resnet(\cite{DBLP:journals/corr/HeZRS15}), VGG (\cite{simonyan2015deep}) and alexnet (\cite{NIPS2012_c399862d}). We are going to use the implementation offered by fastai, which allows us to use a publicly available implementation of those, hence removing possible differences in results coming from divergent implementations of those. Moreover, no further optimization has not been implemented, if not specified. It is therefore assumed for the rest of the paper that, if no specification is made, the training of the models has been carried out by the \textit{fit\_one\_ cycle()} function present in fastai using the default learning rate. Furthermore, the models have not been pre-trained, hence no transferred learning is applied, and the models have been trained using full precision. \\
...
As suspected, for each model, the accuracy grows logarithmically higher as the number of  epochs increments, or as the training time increments. A closer inspection of Fig. n. \ref{fig:com_ti_ac_models} lets us derive other conclusions. First of all, as also demonstrated by authors of \cite{suh_transfer_2018} in different settings, Alexnet finishes training in considerably less time compared to the other networks(\textasciitilde 6 minutes), reaching however the lowest accuracy overall(45\%). We can observe this difference in time by looking at Fig. n.\ref{fig:sing_acc_train2} and Fig. n.\ref{fig:sing_acc_train}, which shows the behaviour of Alexnet, Resnet101, Resnet152 and VGG19 in the same settings. \\
...
Size, however, is not the only property of the image we can analyse in order to find better correlations between the metrics. 
