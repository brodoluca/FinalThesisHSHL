...
minimum height=0.5cm, rounded corners=5pt, draw}}
        ...
        \node[activity, below of = action1] (action2) {Fetch the dataset};
        \node[activity, below of = action2] (action3) {Fetch the models};
        \node[activity, below of = action3] (action4) {Train the models};
        \node[activity, below of = action4] (action5) {Save Results};
        \node[activity, below of = action5] (action6) {Test inference};
        \node[activity, below of = action6] (action7) {Save Results};
        \node[end,below of = action7](end){};
...
\end{figure}
The tool has been written using Python and revolves around the \textit{fastai} library. This library is built on top of Pytorch, a very common library for machine learning in Python, offering powerful function and high flexibility without a significant drop in performances. \cite{fastai}\\
...
\subsection{Analysing the training process}
